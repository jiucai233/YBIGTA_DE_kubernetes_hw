...
> **💡 강력 권장 사항 (GitHub Codespaces):** 
> 이 과제는 **GitHub Codespaces** 사용을 강력히 권장합니다! GitHub 레포지토리에서 `<> Code` 버튼을 누르고 `Codespaces` 탭에서 새로 만들기만 하면 모든 도구(Docker, K3d 등)가 완벽하게 세팅된 클라우드 환경이 브라우저에 열립니다. 환경이 열리면 바로 터미널에서 아래의 과제 명령어를 실행하고 편안히 과제를 즐기시면 됩니다!

이 과제는 Mac, Linux, Windows를 지원합니다. 로컬 PC에서 진행하시려면 운영 체제에 맞는 안내에 따라 필요한 도구를 설치해 주세요.
